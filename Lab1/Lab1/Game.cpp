...
void Game::Render() 
{
	...
	DrawTriangleStack(glm::vec3(-5, 0, 0), 10);
	DrawTriangleStack(glm::vec3(0, 0, 0), 10);
	DrawTriangleStack(glm::vec3(5, 0, 0), 10);
...
void Game::DrawTriangleStack(glm::vec3 s, int numOfTriangles)
{
	for (auto i = 0; i < numOfTriangles; i++)
	{
		float n = i * m_spacing;
		DrawTriangle(glm::vec3(s.x, s.y, s.z + n));
	}
}
